...
def get_query_name_by_query_id(query_id):
    ...
    r_query = requests.get(QUERY_LINK + str(query_id), headers=HEADERS,
                           auth=('', get_ado_token_for_user(sql_api.get_current_user)))
...
def get_test_case_steps_by_url(test_case_url):
    ...
    :return:
    """
    r_query = requests.get(test_case_url, headers=HEADERS, auth=('', get_ado_token_for_user(sql_api.get_current_user)))
    if r_query.status_code == 200:
        r_query.close()
        parsed_data = json.loads(str(r_query.text))
        try:
...
def update_test_steps_in_ado(test_case_sql_id, data):
    ...
    test_case_steps = sql_api.get_test_case_steps_by_id(test_case_sql_id)
    ...
    r_ado = requests.patch(
        ...
        json=json_patch, headers=HEADERS, auth=('', get_ado_token_for_user(sql_api.get_current_user)))
